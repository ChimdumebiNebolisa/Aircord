...
def run_memory_loop(
    ...
    *,
    ...
    try:
        estimate_aqi, blend_basis = reference_blended_estimate(pm25, monitor_aqi, weight)
    except ValueError as exc:
        raise RuntimeError(
            f"Cannot compute an estimate: both PurpleAir PM2.5 and AirNow AQI are missing for {sensor_id}"
        ) from exc
    if pm25 is None:
        estimate_basis = "PurpleAir PM2.5 was missing; used the AirNow monitor AQI as an explicit fallback"
    elif monitor_aqi is None:
        estimate_basis = "AirNow monitor AQI was missing; used the PurpleAir PM2.5 proxy without a reference blend"
    else:
        estimate_basis = (
            f"Blended PurpleAir PM2.5 proxy at weight {weight:.4f} with the AirNow monitor AQI "
            f"at weight {1.0 - weight:.4f} ({blend_basis})"
        )
    ...
    reasoning_text = (
        ...
        f"Estimate basis: Blended PurpleAir PM2.5 proxy with AirNow monitor AQI ({estimate_basis}). "
